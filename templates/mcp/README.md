...
The server then advertises the complete eleven-tool daily-memory surface:
health, compact startup state, latest/recent summaries, explicit session
detail, session creation, search, context preview/load, checkpoints, and
closeout. Project administration remains in the UI and CLI by design.
