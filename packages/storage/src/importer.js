...
const DOCUMENT_STATUSES = ["draft", "active", "accepted", "superseded", "stale", "archived"];
const VISIBILITIES = ["ai-eligible", "ai-pinned", "human-only", "private", "never-send"];
...
export function builtinImportProfiles() {
    return [
        {
...
export function resolveImportProfile(profile) {
    if (!profile)
        return builtinImportProfiles()[0];
...
export async function prepareImportPlan(args) {
    ...
    const candidates = await Promise.all(limitedFiles.map((filePath) => createCandidate({ project: args.project, sourceRoot, filePath, profile })));
    return {
        id: createId("import"),
...
function commonDocumentRules() {
    return [
        { match: "**/README.md", type: "overview" },
        { match: "**/*overview*.md", type: "overview" },
        { match: "**/*architecture*.md", type: "architecture-note" },
        { match: "**/*decision*.md", type: "decision-record" },
        { match: "**/*adr*.md", type: "architecture-decision-record" },
        { match: "**/*plan*.md", type: "plan" },
        { match: "**/*spec*.md", type: "technical-spec" },
        { match: "**/*requirement*.md", type: "requirement" },
        { match: "**/*command*.md", type: "command-note" },
        { match: "**/*gotcha*.md", type: "gotcha" },
        { match: "**/*diagram*.md", type: "diagram" },
        { match: "**/diagrams/**", type: "diagram" }
    ];
}
async function walkImportFiles(root, profile) {
    const files = [];
...
async function createCandidate(args) {
    ...
    const visibility = rule?.visibility || asVisibility(parsed.frontmatter.visibility) || args.profile.defaultVisibility;
    const topics = unique([
    ...
    const targetPath = kind === "skip" ? undefined : importedTargetPath(args.project, args.profile, kind, relativePath);
...
async function commitSession(args) {
    ...
    const session = {
        ...
        workingDirectory: scalar(parsed.frontmatter.working_directory) || args.project.repos[0]?.path || args.plan.sourceRoot,
...
async function targetForCommit(targetPath, sourceHash, strategy) {
...
function importedTargetPath(project, profile, kind, relativePath) {
    const rootFolder = kind === "session" ? "sessions" : "docs";
    return path.join(project.memoryRoot, rootFolder, "imported", filenameSafe(profile.name), safeRelativeMarkdownPath(relativePath));
}
function safeRelativeMarkdownPath(relativePath) {
...
function requiredSourceRoot(sourceRoot) {
    if (!sourceRoot)
        throw new Error("Missing import sourceRoot.");
    return sourceRoot;
}
...
function inferTitle(frontmatter, body, relativePath) {
...
function importedBody(parsedBody, raw, title) {
...
function topicsFromRelativePath(relativePath) {
    const parsed = path.posix.parse(normalizeRelative(relativePath));
    return unique(parsed.dir
        .split("/")
        .filter(Boolean)
        .map((part) => filenameSafe(part))
        .filter(Boolean));
}
function formatFromPath(relativePath) {
...
function asDocumentType(input) {
    return DOCUMENT_TYPES.includes(input) ? input : undefined;
}
function asDocumentStatus(input) {
    return DOCUMENT_STATUSES.includes(input) ? input : undefined;
}
function asVisibility(input) {
    return VISIBILITIES.includes(input) ? input : undefined;
}
function asDocumentFormat(input) {
    return DOCUMENT_FORMATS.includes(input)
        ? input
        : undefined;
}
function asConfidence(input) {
