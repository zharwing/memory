...
const projectIdInput = objectSchema({ projectId: stringSchema });
...
export const OPERATION_REGISTRY = {
  ...
  "memory.get_startup_state": defineOperation({
    input: objectSchema({ workingDirectory: optionalString, projectId: optionalString, clientName: optionalString, knownRevision: optionalString }),
  ...
  "memory.update_memory_write_policy": defineOperation({
    input: objectSchema({ projectId: stringSchema, allowAgentDirectWrites: optionalBoolean, reviewMode: optionalSchema(enumSchema(["off", "risky-only", "all"])) }),
  ...
  "memory.set_provider_secret": defineOperation({
    input: objectSchema({ projectId: stringSchema, providerKind: stringSchema, secret: stringSchema }),
    output: providerSecretStatusSchema,
    effect: "mutation",
    audiences: ["desktop", "admin"],
  ...
  "memory.rotate_provider_secret": defineOperation({
    input: objectSchema({ projectId: stringSchema, providerKind: stringSchema, secret: stringSchema, expectedRevision: stringSchema }),
    output: providerSecretStatusSchema,
    effect: "mutation",
    audiences: ["desktop", "admin"],
    idempotency: "required",
  ...
  "memory.clear_provider_secret": defineOperation({
    input: objectSchema({ projectId: stringSchema, providerKind: stringSchema, expectedRevision: stringSchema }),
    output: providerSecretStatusSchema,
    effect: "mutation",
    audiences: ["desktop", "admin"],
    idempotency: "required",
  ...
  "memory.prepare_destructive_intent": defineOperation({
    input: objectSchema({ projectId: stringSchema, operation: stringSchema, input: jsonObjectSchema }),
    output: destructiveIntentSchema,
    effect: "proposal",
    audiences: ["browser", "desktop", "admin", "backup"]
  }),
  "memory.commit_destructive_intent": defineOperation({
    input: objectSchema({ projectId: stringSchema, intentId: stringSchema, acknowledgement: stringSchema }),
    output: jsonObjectSchema,
    effect: "destructive",
    audiences: ["browser", "desktop", "admin", "backup"],
  ...
  "memory.cancel_destructive_intent": defineOperation({
    input: objectSchema({ projectId: stringSchema, intentId: stringSchema }),
    output: jsonObjectSchema,
    effect: "mutation",
    audiences: ["browser", "desktop", "admin", "backup"]
  }),
  "memory.update_graph_rules": defineOperation({
    input: objectSchema({ projectId: stringSchema, graphRules: optionalSchema(arraySchema(graphExtractionRuleSchema)) }),
  ...
  "memory.link_repo": defineOperation({
    input: objectSchema({ projectId: stringSchema, repoPath: stringSchema, role: optionalString, name: optionalString, description: optionalString, defaultBranch: optionalString, writePointerFile: optionalBoolean }),
    output: jsonObjectSchema,
    effect: "mutation",
    invalidates: ["project-repos", "project-graph", "projects", "project-summary"]
  }),
  "memory.unlink_repo": defineOperation({
    input: objectSchema({ projectId: stringSchema, repoPath: stringSchema, removePointerFile: optionalBoolean }),
    output: jsonObjectSchema,
    effect: "mutation",
    invalidates: ["project-repos", "project-graph", "projects", "project-summary"]
  }),
  ...
  "memory.create_workstream": defineOperation({
    input: objectSchema({ projectId: stringSchema, name: stringSchema, summary: optionalString, goal: optionalString, topics: optionalStrings, repoRoles: optionalStrings, relatedTasks: optionalStrings, relatedFiles: optionalStrings, body: optionalString }),
    output: workstreamSchema,
    effect: "mutation",
    invalidates: ["workstreams", "project-graph"]
  }),
  "memory.get_workstream_detail": defineOperation({ input: objectSchema({ projectId: stringSchema, workstreamId: stringSchema }), output: workstreamDetailSchema, maximumResponseBytes: 4 * 1024 * 1024 }),
  "memory.update_workstream_status": defineOperation({
    input: objectSchema({ projectId: stringSchema, workstreamId: stringSchema, status: enumSchema(["active", "paused", "done", "archived"]) }),
    output: workstreamSchema,
    effect: "mutation",
    invalidates: ["workstreams", "project-graph"]
  }),
  "memory.delete_workstream": defineOperation({ input: objectSchema({ projectId: stringSchema, workstreamId: stringSchema }), output: trashItemSchema, effect: "destructive", invalidates: ["workstreams", "project-graph", "trash"] }),
  "memory.start_session": defineOperation({
    input: objectSchema({ projectId: stringSchema, repoPath: optionalString, workingDirectory: optionalString, branch: optionalString, agent: optionalString, client: optionalString, taskTitle: optionalString, goal: optionalString, workstreamIds: optionalStrings }),
  ...
  "memory.start_or_resume_session": defineOperation({
    input: objectSchema({ projectId: stringSchema, taskTitle: optionalString, workingDirectory: optionalString, branch: optionalString, agent: optionalString, client: optionalString, goal: optionalString }),
    output: sessionSchema,
    audiences: ["desktop", "admin"],
    effect: "mutation",
    invalidates: ["sessions", "project-summary"]
  ...
  "memory.get_recent_sessions": defineOperation({ input: objectSchema({ projectId: stringSchema, limit: optionalSchema(numberRangeSchema(1, 200)) }), output: arraySchema(sessionSummarySchema), audiences: ["desktop", "agent", "admin"], privacyProjection: "agent" }),
  "memory.list_project_sessions": defineOperation({ input: objectSchema({ projectId: stringSchema, limit: optionalSchema(numberRangeSchema(1, 200)) }), output: arraySchema(sessionSummarySchema), maximumResponseBytes: 4 * 1024 * 1024 }),
  "memory.get_session_detail": defineOperation({ input: objectSchema({ projectId: stringSchema, sessionId: stringSchema, sections: optionalSchema(arraySchema(enumSchema(["body", "checkpoints"]))), checkpointLimit: optionalSchema(numberRangeSchema(1, 100)), cursor: optionalString }), output: sessionDetailSchema, audiences: ["browser", "desktop", "agent", "admin"], privacyProjection: "agent", maximumResponseBytes: 4 * 1024 * 1024 }),
  "memory.save_checkpoint": defineOperation({
    input: objectSchema({ projectId: stringSchema, sessionId: stringSchema, summary: stringSchema, nextSteps: optionalStrings, blockers: optionalStrings, touchedFiles: optionalStrings, proposedUpdateIds: optionalStrings, workstreamIds: optionalStrings }),
    output: sessionSchema,
    audiences: ["browser", "desktop", "agent", "admin"],
    privacyProjection: "agent",
    effect: "mutation",
  ...
  "memory.update_session_graph_visibility": defineOperation({ input: objectSchema({ projectId: stringSchema, sessionId: stringSchema, includeInGraph: booleanSchema }), output: sessionSchema, effect: "mutation", invalidates: ["sessions", "project-graph"] }),
  "memory.close_session": defineOperation({
    input: objectSchema({ projectId: stringSchema, sessionId: stringSchema, summary: optionalString, nextSteps: optionalStrings, blockers: optionalStrings, touchedFiles: optionalStrings, workstreamIds: optionalStrings, autoSummarize: optionalBoolean, ...providerFields }),
    output: sessionSchema,
    audiences: ["browser", "desktop", "agent", "admin"],
    privacyProjection: "agent",
    effect: "mutation",
  ...
  "memory.generate_session_summary": defineOperation({ input: objectSchema({ projectId: stringSchema, sessionId: stringSchema, force: optionalBoolean, ...providerFields }), output: jsonObjectSchema, effect: "mutation", invalidates: ["sessions", "project-summary"] }),
  "memory.generate_session_summaries": defineOperation({ input: objectSchema({ projectId: stringSchema, mode: optionalSchema(enumSchema(["missing", "all"])), limit: optionalNumber, ...providerFields }), output: jsonObjectSchema, effect: "mutation", invalidates: ["sessions", "project-summary"] }),
  "memory.delete_session": defineOperation({ input: objectSchema({ projectId: stringSchema, sessionId: stringSchema }), output: trashItemSchema, effect: "destructive", invalidates: ["sessions", "project-summary", "project-graph", "trash"] }),
  "memory.search": defineOperation({ input: objectSchema({ projectId: stringSchema, query: stringSchema, limit: optionalNumber }), output: arraySchema(searchResultSchema), audiences: ["browser", "desktop", "agent", "admin"], privacyProjection: "agent", maximumResponseBytes: 4 * 1024 * 1024 }),
  ...
  "memory.import_doc": defineOperation({ input: objectSchema({ projectId: stringSchema, title: stringSchema, type: stringSchema, body: stringSchema, status: optionalString, visibility: optionalString, topics: optionalStrings, workstreamIds: optionalStrings, relatedTasks: optionalStrings, relatedFiles: optionalStrings, relatedSessions: optionalStrings, format: optionalString }), output: memoryDocumentSchema, effect: "mutation", audiences: ["desktop", "admin"], invalidates: ["documents", "project-summary", "project-graph"] }),
  "memory.create_doc": defineOperation({ input: objectSchema({ projectId: stringSchema, title: stringSchema, type: stringSchema, body: stringSchema, status: optionalString, visibility: optionalString, topics: optionalStrings, workstreamIds: optionalStrings, relatedTasks: optionalStrings, relatedFiles: optionalStrings, relatedSessions: optionalStrings, format: optionalString }), output: memoryDocumentSchema, effect: "mutation", audiences: ["desktop", "admin"], invalidates: ["documents", "project-summary", "project-graph"] }),
  "memory.update_doc": defineOperation({ input: objectSchema({ projectId: stringSchema, documentId: stringSchema, title: optionalString, type: optionalString, body: optionalString, status: optionalString, visibility: optionalString, topics: optionalStrings, workstreamIds: optionalStrings, relatedTasks: optionalStrings, relatedFiles: optionalStrings, relatedSessions: optionalStrings, relatedDiagrams: optionalStrings, format: optionalString, lastVerified: optionalString, confidence: optionalString }), output: memoryDocumentSchema, effect: "mutation", invalidates: ["documents", "project-summary", "project-graph"] }),
  "memory.delete_doc": defineOperation({ input: objectSchema({ projectId: stringSchema, documentId: stringSchema }), output: trashItemSchema, effect: "destructive", invalidates: ["documents", "project-summary", "project-graph", "trash"] }),
  ...
  "memory.prepare_import": defineOperation({ input: objectSchema({ projectId: stringSchema, sourceRoot: stringSchema, profile: stringSchema, limit: optionalNumber }), output: importPlanSchema, maximumResponseBytes: 8 * 1024 * 1024 }),
  ...
  "memory.update_inbox_status": defineOperation({ input: objectSchema({ projectId: stringSchema, proposalId: stringSchema, status: enumSchema(["pending", "accepted", "rejected", "deferred", "edited"]), editedPatch: optionalString }), output: proposedMemoryUpdateSchema, effect: "mutation", invalidates: ["inbox", "project-summary", "documents", "project-graph"] }),
  "memory.delete_inbox_item": defineOperation({ input: objectSchema({ projectId: stringSchema, proposalId: stringSchema }), output: trashItemSchema, effect: "destructive", invalidates: ["inbox", "project-summary", "trash"] }),
  "memory.get_graph": defineOperation({ input: objectSchema({ projectId: stringSchema, includeSemantic: optionalString, includeSemanticProposals: optionalBoolean }), output: projectGraphSchema, maximumResponseBytes: 8 * 1024 * 1024 }),
  "memory.get_semantic_graph_settings": defineOperation({ input: projectIdInput, output: semanticGraphSettingsSchema }),
  "memory.update_semantic_graph_settings": defineOperation({ input: objectSchema({ projectId: stringSchema, settings: optionalSchema(semanticSettingsPatchSchema), enabled: optionalBoolean, mode: optionalString, providerId: optionalString, providerKind: optionalString, model: optionalString, autoAcceptThreshold: optionalNumber, reviewThreshold: optionalNumber, discardBelowThreshold: optionalNumber, maxCandidatesPerDocument: optionalNumber, maxClusterSize: optionalNumber, includeDeterministicSignals: optionalBoolean, includeVectorCandidates: optionalBoolean, remoteProvidersEnabled: optionalBoolean }), output: semanticGraphSettingsSchema, effect: "mutation", invalidates: ["semantic-settings", "semantic-status"] }),
  "memory.get_semantic_graph_status": defineOperation({ input: projectIdInput, output: semanticGraphStatusSchema }),
  "memory.list_semantic_edges": defineOperation({ input: objectSchema({ projectId: stringSchema, status: optionalSchema(unionSchema([stringSchema, strings])) }), output: arraySchema(semanticGraphEdgeSchema), maximumResponseBytes: 8 * 1024 * 1024 }),
  "memory.update_semantic_edge_status": defineOperation({ input: objectSchema({ projectId: stringSchema, edgeIds: strings, status: enumSchema(["proposed", "accepted", "rejected", "auto-accepted"]) }), output: jsonObjectSchema, effect: "mutation", invalidates: ["semantic-edges", "semantic-status", "project-graph"] }),
  ...
  "memory.get_semantic_graph_run": defineOperation({ input: objectSchema({ projectId: stringSchema, runId: stringSchema }), output: semanticGraphRunSchema }),
  "memory.preview_semantic_graph_analysis": defineOperation({ input: objectSchema({ projectId: stringSchema, scope: optionalSchema(semanticGraphScopeSchema), maxDocumentChars: optionalNumber, persistCandidateIndex: optionalBoolean }), output: semanticAnalysisPreviewResultSchema, timeoutMs: 60_000, maximumResponseBytes: 8 * 1024 * 1024 }),
  ...
  "memory.check_semantic_graph_provider": defineOperation({ input: objectSchema({ projectId: stringSchema, ...providerCheckFields }), output: semanticProviderCheckResultSchema, audiences: ["browser", "desktop", "admin", "provider"], timeoutMs: 60_000, privacyProjection: "provider" }),
  "memory.propose_semantic_edges": defineOperation({ input: objectSchema({ projectId: stringSchema, edges: jsonObjectArraySchema, sourceAgent: optionalString }), output: jsonObjectSchema, effect: "proposal", invalidates: ["inbox", "semantic-edges"] }),
  "memory.accept_semantic_edges_proposal": defineOperation({ input: objectSchema({ projectId: stringSchema, proposalId: stringSchema, status: optionalSchema(enumSchema(["accepted", "auto-accepted"])), minConfidence: optionalNumber, maxConfidence: optionalNumber, edgeIndexes: optionalSchema(arraySchema(nonNegativeIntegerSchema)) }), output: jsonObjectSchema, effect: "mutation", invalidates: ["inbox", "semantic-edges", "semantic-status", "project-graph"] }),
  ...
  "memory.delete_backup": defineOperation({ input: objectSchema({ projectId: stringSchema, snapshotPath: stringSchema }), output: trashItemSchema, effect: "destructive", audiences: ["browser", "desktop", "admin", "backup"], invalidates: ["backups", "trash"] }),
  ...
  "memory.restore_trash_item": defineOperation({ input: objectSchema({ projectId: stringSchema, trashItemId: stringSchema }), output: jsonObjectSchema, effect: "destructive", audiences: ["desktop", "admin"], invalidates: ["projects", "trash", "project-content"] }),
  "memory.purge_trash_item": defineOperation({ input: objectSchema({ projectId: stringSchema, trashItemId: stringSchema }), output: jsonObjectSchema, effect: "destructive", audiences: ["desktop", "admin"], invalidates: ["trash"] }),
  "memory.empty_trash": defineOperation({ input: objectSchema({ projectId: stringSchema, trashItemIds: optionalStrings }), output: jsonObjectSchema, effect: "destructive", audiences: ["desktop", "admin"], invalidates: ["trash"] }),
  ...
  "memory.summarize_session": defineOperation({ input: objectSchema({ projectId: stringSchema, sessionId: stringSchema }), output: proposedMemoryUpdateSchema, effect: "proposal", audiences: ["desktop", "admin"], invalidates: ["inbox"] }),
  ...
  "memory.classify_imported_doc": defineOperation({ input: objectSchema({ projectId: stringSchema, documentId: stringSchema }), output: proposedMemoryUpdateSchema, effect: "proposal", audiences: ["desktop", "admin"], invalidates: ["inbox"] }),
