...
export function buildContextBundle(input) {
    ...
    for (const candidate of candidates) {
        ...
        if (!privacy.allowed) {
            if (privacy.excluded)
                excluded.push(privacy.excluded);
            continue;
        }
...
function projectCanonicalCandidates(project, documents) {
...
function documentCandidates(input, query) {
...
function docCandidate(doc, reason, mode = "raw") {
    return {
        id: doc.id,
...
function sessionCandidate(session, mode, reason) {
...
function renderContextMarkdown(projectName, included, excluded, redactions, tokenEstimate) {
