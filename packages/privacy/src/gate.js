...
export function applyPrivacyGate(candidate, policy) {
...
export function combineSafetyStatus(statuses) {
    if (statuses.includes("blocked"))
        return "blocked";
    if (statuses.includes("index-stale"))
        return "index-stale";
    if (statuses.includes("needs-review"))
        return "needs-review";
    return "clean";
}
function exclusionFor(candidate, policy) {
...
function excluded(candidate, reason) {
    return {
        id: candidate.id,
        projectId: candidate.projectId,
        type: candidate.type,
