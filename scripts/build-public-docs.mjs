...
function homeContent() {
  return `<article class="doc-article docs-home-article"><header class="article-header"><p class="article-kicker">Documentation</p><h1>Use, connect, and understand Zharwing Memory.</h1><p class="article-lede">Every guide has a stable page, works without JavaScript, and is generated only from reviewed public repository sources.</p></header><div class="docs-path-grid"><a class="docs-path-card" href="${guidePath("setup")}"><span>01</span><strong>Set up Memory</strong><p>Choose a local profile and start the interface you need.</p><b>Open setup →</b></a><a class="docs-path-card" href="${guidePath("mcp-setup")}"><span>02</span><strong>Connect an AI agent</strong><p>Use the bounded project-scoped MCP surface.</p><b>Open MCP setup →</b></a><a class="docs-path-card" href="${guidePath("architecture-decisions")}"><span>03</span><strong>Review decisions</strong><p>Understand the durable architecture and its tradeoffs.</p><b>Open decisions →</b></a></div><section class="all-guides"><p class="article-kicker">All guides</p><h2>Browse by task.</h2><div class="all-guides-grid">${groups.map((group) => `<section><h3>${escapeHtml(group.title)}</h3>${group.docs.map(([, slug, title, description]) => `<a href="${guidePath(slug)}"><strong>${escapeHtml(title)}</strong><span>${escapeHtml(description)}</span></a>`).join("")}</section>`).join("")}</div></section></article>`;
}
